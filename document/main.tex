...
\subsection{Filtro rechaza bandas}
Un filtro rechaza bandas es un tipo de filtro diseñado para atenuar o eliminar una rango específico de frecuencias, mientras que deja pasar tanto las frecuencias más bajas como las más altas. Es útil cuando se desea suprimir una banda de frecuencias no deseadas, como ruido o interferencias, sin afectar el resto del contenido espectral \cite{oppenheim2010}.

La función principal de este filtro es rechazar una banda central de frecuencia \( f_c \), definida entre dos frecuencias: la frecuencia de corte inferior \textit{ \( f_L \)} y la frecuencia de corte superior \textit{ \( f_H \)}. El ancho de la banda eliminada se conoce como el ancho de banda de rechazo y se calcula de la siguiente manera:
