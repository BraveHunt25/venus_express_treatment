...
\begin{document}
    \begin{titlepage}
        ...
        \begin{tabular}{@{}m{0.2\textwidth}@{}m{0.6\textwidth}@{}m{0.2\textwidth}@{}}
            \centering\raisebox{-\depth}{\includegraphics[width=2.5cm, height=3cm]{assets/ipn (1).jpeg}} & % Tamaño fijo y alineación
            \centering \textsc{\textbf{\LARGE Instituto Politécnico Nacional}} & 
            \centering\raisebox{-\depth}{\includegraphics[width=2.5cm, height=3cm]{assets/escom (1).jpeg}} \\ % Tamaño fijo y   alineación
        \end{tabular}
        ...
        {\fontsize{20pt}{20pt}\selectfont \textbf{Proyecto}\\[0.5 cm] Procesamiento y Análisis de Señales Magnéticas de Venus: Aplicación de Técnicas Digitales al Magnetómetro MAG de Venus Express }\\[3cm]
...
\newpage
        \subsection{Organización del documento}
            Este documento está estructurado en varias secciones para facilitar la comprensión del desarrollo y resultados del proyecto:

            En primer lugar, el \textbf{marco teórico} presenta los conceptos fundamentales sobre el entorno espacial de Venus, la misión \textit{Venus Express}, la instrumentación utilizada (especialmente el magnetómetro MAG) y las herramientas matemáticas y de procesamiento digital de señales empleadas, como la Transformada Discreta de Fourier y los filtros pasa bajas.

            A continuación, la sección de \textbf{metodología} describe detalladamente el proceso seguido para la adquisición, selección, visualización y procesamiento de los datos magnéticos. Se explica la justificación de la elección del sistema de referencia, las técnicas de filtrado aplicadas y el cálculo de la magnitud total del campo magnético.

            Posteriormente, en la sección de \textbf{resultados}, se muestran las gráficas y análisis obtenidos en cada etapa del procesamiento, incluyendo la comparación entre la señal original y la señal filtrada, así como la validación frente a los datos de referencia de la Agencia Espacial Europea.

            Finalmente, en las \textbf{conclusiones}, se realiza una interpretación física de los resultados, se evalúa la efectividad de los métodos de procesamiento aplicados y se discuten posibles aplicaciones y mejoras futuras para el análisis de señales magnéticas en entornos planetarios.
\newpage
    % \newpage
...
Se optó por trabajar con las señales BOSX, BOSY y BOSZ, correspondientes al sensor externo del magnetómetro. Esto se debe a que el sensor BOS se encuentra alejado del fuselaje de la nave, por lo que sus lecturas presentan menor contaminación electromagnética en comparación con el sensor BIS. Además, permite una medición más confiable del campo magnético ambiental. 
	...
	Para la visualización de los datos obtenidos por los sensores, se empleó el lenguaje de programación Python, ampliamente utilizado en el ámbito científico por su versatilidad y facilidad de uso. En particular, se utilizaron las bibliotecas \textit{Matplotlib} para la generación de gráficos y \textit{NumPy} para la manipulación eficiente de arreglos multidimensionales y el manejo de marcas temporales. El código fuente desarrollado para este análisis se encuentra disponible en el repositorio mencionado en \cite{repogithub}.

        \subsection{Procesamiento de la señal}
	Aplicando los filtros y herramientas de análisis proporcionados por la biblioteca \textit{NumPy}, se inicia el proceso con un análisis espectral de los componentes BOSX, BOSY y BOSZ. Este análisis permite identificar las frecuencias predominantes en cada señal. Con base en esta información, se aplica un filtro pasa bajas para atenuar el ruido de alta frecuencia y obtener una primera versión depurada de la señal.

        A continuación, se realiza una segunda inspección espectral mediante la Transformada Rápida de Fourier (FFT), lo que permite evaluar la eficacia del filtrado inicial. Finalmente, se aplica un filtro rechaza banda diseñado para eliminar componentes de frecuencia específicas, con el objetivo de ajustar la señal filtrada lo más posible a las características de la señal original.

        \subsection{Cálculo del modulo total BOST}
        Una vez aplicados los filtros correspondientes a cada componente de la señal, se procede al cálculo del módulo total del campo magnético en el sistema BOS. Este valor se obtiene mediante la raíz cuadrada de la suma de los cuadrados de cada componente individual.

        \subsection{Comparación con BOST de la ESA}
        Finalmente, con los resultados obtenidos, se realizará la comparación entre la señal procesada con los filtros y la señal BOST proporcionada originalmente para, entonces, interpretar la señal. La magnitud resultante permite representar una señal unidimensional que integra la información vectorial del campo, la cual se grafica con el fin de facilitar su análisis e interpretación comparativa frente a otras referencias.

    \section{Resultados}
        \subsection{Adquisición de datos}
            En las Figuras \ref{fig:bosx-original}, \ref{fig:bosy-original} y \ref{fig:bosz-original} se pueden observar las gráficas correspondientes a las señales originales de las componentes BOSX, BOSY y BOSZ, respectivamente. Como se ha expuesto en capítulos anteriores, estas señales presentan un nivel significativo de ruido en las frecuencias altas, lo cual dificulta una interpretación directa del comportamiento físico del campo magnético inducido en el entorno de Venus.

            No obstante, a pesar del ruido presente, es posible identificar con claridad las principales tendencias de variación, las cuales se manifiestan predominantemente en el rango de frecuencias bajas. Estas variaciones reflejan las dinámicas de gran escala asociadas al entorno magnetoplasmático del planeta.
        ...
        \subsection{Procesamiento de señales}
           En las Figuras \ref{fig:fft-bosx-complete}, \ref{fig:fft-bosy-complete} y \ref{fig:fft-bosz-complete} se presentan las transformadas discretas de Fourier correspondientes a las señales originales de las componentes BOSX, BOSY y BOSZ, respectivamente. Estos espectros de frecuencia permiten analizar la distribución energética de cada señal en el dominio de las frecuencias.

            Como se puede observar, en los tres casos la mayor parte de la energía se concentra en frecuencias muy cercanas a cero. Este resultado confirma que las principales variaciones en las señales ocurren en el rango de bajas frecuencias, lo cual es coherente con la naturaleza del campo magnético inducido en el entorno de Venus, caracterizado por procesos lentos y de gran escala.
...
En las Figuras \ref{fig:fft-bosx-partial}, \ref{fig:fft-bosy-partial} y \ref{fig:fft-bosz-partial} se muestran las representaciones espectrales de las componentes BOSX, BOSY y BOSZ, limitadas a los primeros 150 componentes de frecuencia obtenidos mediante la transformada discreta de Fourier. Esta visualización parcial permite realizar un análisis más detallado del contenido de bajas frecuencias en cada señal.

            Se observa que, tanto en BOSY como en BOSZ, la mayor parte de la energía se concentra en frecuencias muy cercanas a cero, con valores que no superan los $2.5 \cdot 10^{-4}$ Hz. En contraste, la componente BOSX presenta un espectro más amplio, con múltiples componentes significativos en un rango mayor de frecuencias. Esto indica que la señal BOSX contiene un nivel más alto de contaminación espectral, lo cual sugiere una mayor presencia de ruido o interferencias en dicha componente.
 	...
 	En las Figuras \ref{fig:bosx-hff}, \ref{fig:bosy-hff} y \ref{fig:bosz-hff} se presentan los resultados obtenidos tras aplicar un filtro pasa bajas a las componentes BOSX, BOSY y BOSZ, eliminando las frecuencias superiores a $5 \cdot 10^{-5}$ Hz. Esta decisión se tomó con el objetivo de mantener la congruencia entre los componentes de frecuencia de las tres señales, así como de preservar los mínimos y máximos globales presentes en la forma de onda original.

        La aplicación de este filtro permite atenuar significativamente el ruido de alta frecuencia, conservando únicamente las variaciones de baja frecuencia que contienen la información más relevante del campo magnético inducido en el entorno de Venus.
            ...
            En las Figuras \ref{fig:fft-hff-bosx-complete}, \ref{fig:fft-hff-bosy-complete} y \ref{fig:fft-hff-bosz-complete} se muestran los espectros de frecuencia completos de las señales filtradas correspondientes a las componentes BOSX, BOSY y BOSZ, respectivamente. Estas señales, una vez procesadas, presentan un comportamiento más coherente con lo esperado para un campo magnético inducido.
	  ...
	  En las Figuras \ref{fig:fft-hff-bosx-partial}, \ref{fig:fft-hff-bosy-partial} y \ref{fig:fft-hff-bosz-partial} se ilustran los primeros 25 componentes de frecuencia de las señales filtradas para las tres componentes del campo magnético: BOSX, BOSY y BOSZ. Esta visualización parcial permite examinar con mayor precisión el contenido espectral más relevante, concentrado en las frecuencias bajas.

Durante el proceso de análisis se experimentó con la aplicación de filtros adicionales para intentar eliminar remanentes de ruido. Sin embargo, se observó que cualquier eliminación más agresiva de componentes de frecuencia afectaba negativamente la estructura de la señal, comprometiendo la integridad de la información asociada al fenómeno físico. Por lo tanto, se concluyó que el nivel actual de filtrado representa un equilibrio adecuado entre reducción de ruido y preservación de las características significativas del campo magnético inducido en Venus.
	  ...
	  Posteriormente al proceso de limpieza de las señales individuales, se procedió al cálculo de la magnitud total del campo magnético a lo largo del tiempo, obtenida como la raíz cuadrada de la suma de los cuadrados de las tres componentes filtradas. En la Figura \ref{fig:bost-filtered} se presenta la señal BOST resultante tras el proceso de filtrado, la cual refleja de manera más clara las variaciones globales del campo, libre de las perturbaciones de alta frecuencia que afectaban las señales originales.
	 ...
	 Finalmente, con el objetivo de resaltar el impacto del proceso de filtrado, en la Figura \ref{fig:bost-comparisson} se presenta una comparación directa entre la señal BOST original y su versión filtrada. Esta superposición permite observar con claridad la atenuación del ruido de alta frecuencia y la conservación de las tendencias generales del campo magnético inducido.

La figura evidencia cómo el filtrado mejora la legibilidad de la señal sin comprometer la estructura principal del fenómeno físico, facilitando así un análisis más preciso del comportamiento del campo magnético en el entorno de Venus.
    ...
    \section{Conclusiones}
	  \subsection{Interpretación de la señal resultante}
                ...
                La señal procesada revela que el campo magnético en Venus no es generado de manera interna, sino que responde directamente a la interacción con la actividad solar. Esto se observa al analizar los valores registrados: cuando el satélite se encuentra en la cara de Venus expuesta al Sol, las lecturas del campo magnético aumentan significativamente; en cambio, al pasar por la cara oculta, los valores descienden hasta volverse casi nulos.
                
                Este patrón indica que el campo magnético es inducido por el viento solar al interactuar con la ionósfera venusina, en lugar de provenir de un mecanismo interno, tal como ocurre en la Tierra. Por lo tanto, se concluye que Venus no posee un campo magnético propio, sino uno inducido externamente, dependiente de las condiciones solares.

  
	  \subsection{Aplicación de filtros}
	  	  Las herramientas teóricas y computacionales estudiadas a lo largo del semestre permitieron analizar, procesar y filtrar adecuadamente la señal obtenida por los sensores, logrando así reflejar de manera más clara el comportamiento de un fenómeno físico real.
            
            Este resultado valida la utilidad de los métodos trabajados en clase, al demostrar que es posible diseñar procedimientos efectivos para el tratamiento y la adquisición de señales discretas muestreadas mediante instrumentos digitales. En particular, el uso de filtros adecuados y el análisis en el dominio de la frecuencia permitieron destacar las características relevantes del campo magnético inducido en Venus, eliminando componentes no deseadas y facilitando su interpretación científica.
